...
pts += -ticky' > _ticky/hadrian.settings; hb --flavour=validate --build-root=_ticky";
  ...
  hsdl = "nix-shell -E '(import <nixpkgs> {}).haskellPackages.developPackage {root = ./.; modifier = drv: pkgs.haskell.lib.addBuildTools drv (with pkgs.haskellPackages; [ haskell-language-server ]);}'";
